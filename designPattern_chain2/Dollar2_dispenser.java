...
public class Dollar2_dispenser implements Chain{

    public Dollar2_dispenser() {
    ...
    public void dispense(Currency req_Amount) {
        if (req_Amount.getCurrency() == "USD") {
            if (req_Amount.getAmount() >= 2) {
                int count = req_Amount.getAmount() / 2;
                int remaining = req_Amount.getAmount() % 2;
                System.out.println("Dispensing " + count + " $2 note(s)");
                if (remaining != 0) {
